 ...
 * JTIs are replay-recorded at every level on success.
 * { mandateJws, expectedAudience = null }
 */
export function verifyChain({ mandateJws, expectedAudience = null }) {
  ...
  for (let depth = 0; depth <= 8; depth++) {
    ...
    if (replay[p.jti]) return fail(`link ${depth}: replay detected`);
    ...
    if (gp.aud === AGENCY_AUD) {
      // Root: the owner's agency grant.
      if (replay[gp.jti]) return fail("agency grant replay detected");
      replay[gp.jti] = gp.exp;
      chain.push({
        issuer: gp.iss,
